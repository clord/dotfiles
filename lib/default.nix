...
let
  ...
  mkNixosConfiguration = system: host: cfg:
    ...
    mkDarwinConfiguration = system: host: cfg@{ os, cpu, admins, host_pubkey, roles, users }:
...
in
{
  ...
  mkHomeConfigurations = list: builtins.listToAttrs (map (username:
        (lib.attrsets.nameValuePair username (mkHomeConfiguration username))) list);
